...
class PendingUsers(LoginRequiredMixin, SuperUserCheck, ListView):
    ...
    def get_queryset(self):
        ...
        if query:
            ...
            ).order_by("is_assigned").exclude(user__username=self.request.user)
        else:
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["form"] = SearchForm
        context["users_pending"] = Profile.objects.filter(is_assigned=False).exclude(user__username=self.request.user).count()
        context["users_assigned"] = AdminScheduledConsultation.objects.filter(user__is_assigned=True).exclude(managed_by=Profile.objects.get(user__username=self.request.user)).count()
        return context
...
class UserDetailView(SuperUserCheck,FormMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            obj = AdminScheduledConsultation.objects.get(user__user__username=self.kwargs.get('user'))
            context['admin'] = obj
            if obj.is_done == True:
...
def approve_user(request, user, pk):
    try:
        ...
        admin = AdminScheduledConsultation(user=user_obj, managed_by=managed_by, is_done = True)
        admin.save()

        pass

    try:
        profile = Profile.objects.get(user__username=user, id=pk)
...
class UserSchedules(LoginRequiredMixin, SuperUserCheck, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        soon = queryset.filter(scheduled_date__date=now.date(), scheduled_date__time__gt=now.time(), user__is_assigned=True)
        upcoming = queryset.filter(scheduled_date__date__gt=now.date(), user__is_assigned=True)
